...
fn train_model(args: PythonObject) raises -> None:
    ...
    var trainer = Python.evaluate("None")  # Initialize trainer to None
    var tracker: PythonObject = Python.evaluate("None")
    var trainer_py: PythonObject = Python.evaluate("None")
    var study: PythonObject = Python.evaluate("None")  # Initialize study to None
    var profiler: PythonObject = Python.evaluate("None")  # Default to None
    ...
    try:
        if args.use_optuna:
            ...
            # Check if study_name is None
            if study_name.is_(Python.evaluate("None")):
        ...
        # Step 2: Ensure study was assigned before using best_params
        if not study.is_(Python.evaluate("None")):
            ...
            # Default parameters if study is not used or failed to load/create
        ...
        # Get the Config and base configuration types
        var ModelConfig = Python.import_module("gpt2_arc.src.config").ModelConfig
        var TrainingConfig = Python.import_module("gpt2_arc.src.config").TrainingConfig
        ...
        var debug_msg = Python.evaluate("'Configuration: {}'").format(config)
        logger.debug(debug_msg)

        # Declare all_synthetic_data at the outer scope
        var all_synthetic_data: PythonObject
        if args.use_synthetic_data:
            all_synthetic_data = load_and_split_synthetic_data(args, config)
        else:
            all_synthetic_data = Python.evaluate("None")
        ...
        # Use Python string formatting for info messages
        var train_source_msg = Python.evaluate("'Training dataset source: {}'").format(
            "synthetic data" if args.use_synthetic_data else "official ARC data"
        )
        logger.info(train_source_msg)
        ...
        var symbol_freq_dict = Python.dict()  # Create empty Python dictionary
        ...
        if args.model_checkpoint:
            # Use Python string formatting for logging
            var checkpoint_msg = Python.evaluate("'Loading model from checkpoint: {}'").format(args.model_checkpoint)
            logger.info(checkpoint_msg)
            var checkpoint = torch.load(args.model_checkpoint)
            
            # Use Python's 'in' operator through evaluate
            var has_configs = Python.evaluate("'model_config' in checkpoint and 'training_config' in checkpoint").to_bool()
            
            if has_configs:
                var ModelConfig = Python.import_module("gpt2_arc.src.config").ModelConfig
                var TrainingConfig = Python.import_module("gpt2_arc.src.config").TrainingConfig
                
                # Convert the dictionary unpacking to Python evaluation
                var model_config = Python.evaluate("ModelConfig(**checkpoint['model_config'])")
                var training_config = Python.evaluate("TrainingConfig(**checkpoint['training_config'])")
                ...
                var debug_msg = Python.evaluate("'Loaded TrainingConfig with num_classes={} from checkpoint'").format(num_classes)
                logger.debug(debug_msg)
            else:
                logger.error("Checkpoint missing 'model_config' or 'training_config'.")
                raise Error("Checkpoint must contain 'model_config' and 'training_config'.")
        ...
        var trainer_py = ARCTrainer(
            model=model,
            train_dataset=train_data,  # Now properly scoped
        ...
        # Declare the variables at the top level with default values
        var accelerator = "cpu"
        var devices = 1
        var strategy = "auto"
        ...
        if args.accelerator == "tpu":
            accelerator = "tpu"
            var devices: PythonObject = 1  # Default integer value
            if args.accelerator == "tpu":
                devices = Python.evaluate("'xla:1'")  # Use 'xla:8' for TPU v3-8 pods
            elif args.accelerator == "gpu":
                if torch.cuda.is_available():
                    devices = 1  # Can remain an integer for GPU devices
        ...
        var callbacks = Python.evaluate("[]")  # Create a native Python list
        ...
        if not args.no_checkpointing:
            ...
            config_saver.set_config(config)
            # Convert Mojo struct to Python object before appending
            var py_config_saver = config_saver.convert_to_python()
            callbacks.append(py_config_saver)
            # Remove the ModelConfigSaver line - we don't need it
        ...
        if not args.no_logging:
            var tb_logger: PythonObject = Python.evaluate("None")  # Set default to None
        ...
        if args.use_gpu and torch.cuda.is_available():
            ...
            var log_message = Python.evaluate("'Initial CUDA memory allocated: {} bytes'").format(memory_allocated)
            logger.info(log_message)
            
            var memory_reserved_str = Python.evaluate("str")(memory_reserved)
            var log_message_reserved = "Initial CUDA memory reserved: " + memory_reserved_str + " bytes"
            logger.info(log_message_reserved)
                    
        logger.info("Starting model training")
        var log_message = Python.evaluate("'Training parameters: batch_size={}, learning_rate={}, max_epochs={}'").format(
        ...
        if args.use_gpu and torch.cuda.is_available():
            var memory_allocated = torch.cuda.memory_allocated()
            var memory_reserved = torch.cuda.memory_reserved()
            var log_message_allocated = Python.evaluate("'CUDA memory allocated after training: {} bytes'").format(memory_allocated)
            var log_message_reserved = Python.evaluate("'CUDA memory reserved after training: {} bytes'").format(memory_reserved)
            
            logger.info(log_message_allocated)
            logger.info(log_message_reserved)
        ...
        if test_results:
            ...
            for result in test_results:
                sum_test_loss += Float64(result["avg_test_loss"])
                sum_test_accuracy += Float64(result["avg_test_accuracy"])
                sum_test_diff_accuracy += Float64(result["avg_test_diff_accuracy"])
            ...
            var log_message = Python.evaluate("'Test results - Loss: {} bytes, Accuracy: {}, Diff Accuracy: {}'").format(
                avg_test_loss,
                avg_test_accuracy,
            ...
            var results = Python.evaluate("dict()")
            results["avg_test_loss"] = avg_test_loss
            results["avg_test_accuracy"] = avg_test_accuracy
            results["avg_test_diff_accuracy"] = avg_test_diff_accuracy
            ...
            var final_metrics = Python.evaluate("dict()")
            final_metrics.update("best_val_loss", trainer_py.best_val_loss)
            final_metrics.update("best_epoch", trainer_py.best_epoch)
            final_metrics.update("final_test_loss", avg_test_loss)
            final_metrics.update("final_test_accuracy", avg_test_accuracy)
            final_metrics.update("final_test_diff_accuracy", avg_test_diff_accuracy)
            
            trainer_py.results_collector.set_final_metrics(final_metrics)
            
        
        # Save the final model with configuration
        logger.info("Saving final model with configuration")
        var model_path = Python.evaluate("'final_model_{}_pth'.format").format(trainer_py.results_collector.experiment_id)
        var os_module = Python.import_module("os")
        os_module.makedirs("checkpoints", exist_ok=True)
        var checkpoint_dict = Python.evaluate("dict()")
        checkpoint_dict.update("state_dict", trainer_py.model.state_dict())
        checkpoint_dict.update("model_config", trainer_py.config.model.__dict__)
        checkpoint_dict.update("training_config", trainer_py.config.training.__dict__)
        checkpoint_dict.update("pad_symbol_idx", trainer_py.config.training.pad_symbol_idx)
        checkpoint_dict.update("symbol_freq", trainer_py.config.training.symbol_freq)
        ...
        var results_path = Python.evaluate("'results/experiment_{}.json'").format(trainer_py.results_collector.experiment_id)
        ...
        var error_info = Python.evaluate("""
        error_str = str(sys.exc_info()[1])
        is_cuda_error = 'CUDA out of memory' in error_str
        {'is_cuda': is_cuda_error, 'message': error_str}
        """)
        
        if error_info["is_cuda"].to_bool():
            logger.error("CUDA out of memory error occurred.")
            logger.error("Consider reducing the batch size or model complexity.")
        else:
            logger.error("Unknown runtime error occurred.")
        # Instead of bare raise, throw a specific error
        raise Error("Training failed: " + error_info["message"].__str__())

        
    finally:
        # Check if tracker is not None
        if not tracker.is_(Python.evaluate("None")):
            tracker.finish()
    

    if not trainer_py.is_(Python.evaluate("None")):
